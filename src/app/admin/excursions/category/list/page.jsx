...
function Page() {
    ...
    const [filteredexcursion, setfilteredexcursion] = useState([]); // Initialize with an empty array
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            // Fetch excursions
            await axios.get(`http://localhost:5000/api/excursions/categories`)
                .then((result) => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.delete(`http://localhost:5000/api/excursions/categories/${id}`);
    ...
    const displayedEmployee = filteredexcursion.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredexcursion(excursion); // Reset filtered data to all data
        } else {
        ...
        <div className="flex flex-col md:flex-row h-screen">
            ...
            <div className="flex-1 overflow-auto bg-gray-100">
                ...
                <div className='p-4 bg-white mx-5 mt-3 rounded-lg shadow-lg'>
                    <div>
                        <h2 className="text-2xl font-medium mb-3 text-black">Categories List</h2>
                        <div className='flex justify-between items-center mb-4'>
                            <div className='flex'>
                                <Link href="../../../admin/excursions/category/add" className='text-sm w-28 h-8 flex items-center justify-center border border-blue-600 bg-white mr-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-sm' passHref>
                            ...
                            <table className="w-full bg-white rounded-lg shadow-lg overflow-hidden">
                                <thead className="bg-blue-400">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-300 text-white" style={{ width: "40%" }}>Icon</th> {/* Updated header */}
