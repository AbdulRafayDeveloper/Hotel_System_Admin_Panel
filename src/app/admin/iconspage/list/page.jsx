...
function Page() {
    ...
    const [filteredicon, setfilteredicon] = useState([]); // Initialize with an empty array
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            await axios.get(`http://localhost:5000/api/icons`)
                .then((result) => {
                    console.log("result.data: ", result.data);
                    if (Array.isArray(result.data)) {
                        console.log("result.data.data: ", result.data);
                        seticon(result.data);
                        setfilteredicon(result.data);
                    } else {
    ...
    const displayedEmployee = filteredicon.slice(startIndex, endIndex);

    const handleFilter = (e) => {
        const searchText = e.target.value.toLowerCase();

        if (searchText.trim() === '') {
            setfilteredicon(icon); // Reset filtered data to all data
        } else {
            const filteredData = icon.filter((item) =>
                Object.values(item).some((value) =>
                    String(value).toLowerCase().includes(searchText)
                )
            );
            setfilteredicon(filteredData);
        }
                    ...
                    <div>
                        ...
                        <div className="flex justify-center mt-4">
                            <ReactPaginate
                                ...
                                pageCount={Math.ceil(icon.length / perPage)}
